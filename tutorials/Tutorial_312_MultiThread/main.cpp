...
// This example shows how to use another thread for modifying the model and notify the viewer thread.


void edit_model(PointCloud *cloud, Viewer *viewer) {
    // modify the model
    // in this example, we add more points (with per point colors) to the point cloud.
    ...
    // notify the viewer to update the visualization
    viewer->update();
}
...
int main(int argc, char **argv) {
    ...
    Timer<>::single_shot(0, [&]() {
        ...
        for (int i = 0; i < 50; ++i) {
            // allow sufficient time for each edit (each edit runs in a separate thread)
