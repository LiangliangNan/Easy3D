...
namespace easy3d {
	...
	void ShaderProgram::clear() {
		...
		for (int i = 0; i < ShaderProgram::NUM_SHADER_TYPES; ++i) {
			if (pShader[i]) {
				...
				glDetachShader(program_, pShader[i]);	
	...
	bool ShaderProgram::load_shader_from_code(ShaderType type, const std::string& str) {
		...
		GLuint shader = glCreateShader(spGLShaderTypes[type]);	
	...
	bool ShaderProgram::shader_info_log(std::string& log, GLuint shader) {
		...
		if (compile_status != GL_TRUE) {
			...
			glGetShaderInfoLog(shader, log_length, 0, &log[0]);	
			return false;
		}
		else
			return true;
	...
	bool ShaderProgram::is_shader_compiled(ShaderType aType) {
		GLint b = GL_FALSE;
		if (pShader[aType]) {
			glGetShaderiv(pShader[aType], GL_COMPILE_STATUS, &b);	
		}
		return (b != GL_FALSE);
	}
	...
	bool ShaderProgram::is_attribute_used(const std::string& name) {
		...
		GLint loc = glGetAttribLocation(program_, name.c_str());	
		return (loc != -1);
	}
	...
	void ShaderProgram::print_active_attributes() {
		...
		for (int i = 0; i < numAttribs; ++i) {
			GLint results[3];
			glGetProgramResourceiv(program_, GL_PROGRAM_INPUT, i, 3, properties, 3, NULL, results);	

			GLint nameBufSize = results[0] + 1;
			char * name = new char[nameBufSize];
			glGetProgramResourceName(program_, GL_PROGRAM_INPUT, i, nameBufSize, NULL, name);	
	...
	void ShaderProgram::print_active_uniforms() {
		...
		for (int i = 0; i < numUniforms; ++i) {
			GLint results[4];
			glGetProgramResourceiv(program_, GL_UNIFORM, i, 4, properties, 4, NULL, results);	
			if (results[3] != -1)
				continue;  // Skip uniforms in blocks 
			GLint nameBufSize = results[0] + 1;
			char * name = new char[nameBufSize];
			glGetProgramResourceName(program_, GL_UNIFORM, i, nameBufSize, NULL, name);	
			std::cout << "   " << results[2] << "\t" << name << "\t" << _type_string(results[1]) << "" << std::endl;
			delete[] name;
		}
	}
	...
	void ShaderProgram::print_active_uniform_blocks() {
		...
		for (int block = 0; block < numBlocks; ++block) {
			GLint blockInfo[2];
			glGetProgramResourceiv(program_, GL_UNIFORM_BLOCK, block, 2, blockProps, 2, NULL, blockInfo);	
			GLint numUnis = blockInfo[0];

			char * blockName = new char[blockInfo[1] + 1];
			glGetProgramResourceName(program_, GL_UNIFORM_BLOCK, block, blockInfo[1] + 1, NULL, blockName);	
			std::cout << "Uniform block \"" << blockName << "\"\t" << std::endl;
			delete[] blockName;

			GLint * unifIndexes = new GLint[numUnis];
			glGetProgramResourceiv(program_, GL_UNIFORM_BLOCK, block, 1, blockIndex, numUnis, NULL, unifIndexes);	

			for (int unif = 0; unif < numUnis; ++unif) {
				GLint uniIndex = unifIndexes[unif];
				GLint results[3];
				glGetProgramResourceiv(program_, GL_UNIFORM, uniIndex, 3, props, 3, NULL, results);	

				GLint nameBufSize = results[0] + 1;
				char * name = new char[nameBufSize];
				glGetProgramResourceName(program_, GL_UNIFORM, uniIndex, nameBufSize, NULL, name);	
				std::cout << "   " << results[2] << "\t" << name << "\t" << _type_string(results[1]) << "" << std::endl;
				delete[] name;
			}
	...
	void ShaderProgram::_add_blocks() {
		...
		for (int i = 0; i < count; ++i) {
			...
			glGetActiveUniformBlockName(program_, i, actualLen, NULL, name);	
	...
	void  ShaderProgram::_add_uniforms() {
		...
		for (int i = 0; i < count; ++i) {
			...
			if (loc != -1) {
				glGetActiveUniformsiv(program_, 1, (GLuint*)&i, GL_UNIFORM_ARRAY_STRIDE, &uniArrayStride);	
