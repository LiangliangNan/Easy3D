...
namespace easy3d {
    ...
    *			  transform vectors that are M-1 in size, as vectors in	homogeneous space. */
    ...
    //	------- global matrix-scalar arithmetic operators --------
    ...
    *	used with luBackSubstitution() to solve a set of linear equations or
    ...
    *	return true on success, false if a is a singular matrix.*/
    ...
    /**	Solve a set of linear equations. Use outputs from luDecomposition() as inputs.
    ...
    *	...
    *	luDecomposition(a, &alu, &rowp, &d);	// get lu decomposition
    *	luBackSubstitution(alu, rowp, b, &x);	// get solution set
    *	@endcode
    ...
    *	T d;	// sign of determinant
    *	...
    *	luDecomposition(a, &alu, &rowp, &d); // get lu decomposition once
    *	for (size_t i = 0; i < N; ++i) {	// find inverse by columns
    *	Vec<N, T> b;
    *	for (size_t j = 0; j < N; ++j)
    *	b[j] = T(0);
    *	b[i] = T(1);
    *	luBackSubstitution(alu, rowp, b, &b);
    ...
    *	luDecomposition(a, &alu, &rowp, &d);
    *	for (size_t i = 0; i < N; ++i)
    *		d *= alu(i, i);
    *	@endcode
    *	alu: N x N matrix, the result of a call to luDecomposition()
    ...
    *******************************************************************************/
    ...
    inline T determinant(const Mat<N, N, T> &m) {
        ...
        Vec<N, size_t> rowp;
        T result;
        luDecomposition(m, &tmp, &rowp, &result);
        for (size_t i = 0; i < N; ++i)
            result *= tmp(i, i);
        return result;
    }
    ...
    template <size_t N, size_t M, typename T>
    inline bool gaussJordanElimination(
        const Mat<N, N, T> &a,
        const Mat<N, M, T> &b,
        Mat<N, N, T> *ainv,
        Mat<N, M, T> *x) {
    ...
    /*	Adapted from "Numerical Recipes in C" (Press et al) */
    template <size_t N, typename T>
    inline void luBackSubstitution(
        const Mat<N, N, T> &alu,
        const Vec<N, T> &rowp,
        const Vec<N, T> &b,
        Vec<N, T> *x
    ...
    *	T: The scalar type for vector elements. See Mat */
    ...
    public:
        ...
        *	other elements are initialized as the identity. */
        ...
        /**	Initialize as a 3D rotation matrix from a rotation quaternion q.
        *	NOTE: q is a unit quaternion (i.e. a rotation). */
        explicit Mat3(const Quat<T> &q);

        // the upper-left 2x2 sub-matrix,
        Mat2<T> sub() const;

        /**	Static constructor return a 3D uniform scale matrix.
        *	s: x, y, z scale. */
        static Mat3<T> scale(T s);

        /**	Static constructor return a 3D non-uniform scale matrix.
        *	x,y,z: x, y, z scale. */
        static Mat3<T> scale(T x, T y, T z);

        /**	Static constructor return a 3D rotation matrix.
        *	axis: Axis of rotation. This is assumed to be normalized.
        *	angle: Angle of rotation in radians.
        *	NOTE: Positive values of angle rotate counter-clockwise about
        *	axis as per the right-hand rule.*/
    ...
    *******************************************************************************/
    ...
    assert(std::abs(q.length() - 1) < std::numeric_limits<T>::epsilon());
        ...
        (*this).set_row(0, m1.row(0));
        (*this).set_row(1, m1.row(1));
        (*this).set_row(2, m1.row(2));
    }
    ...
    *	T: The scalar type for vector elements. See Mat */
    ...
    public:
        ...
        *	other elements are initialized as the identity. */
        ...
        /**	Static constructor return a 4D uniform scale matrix.
        *	s: x, y, z, w scale. */
        static Mat4<T> scale(T s);

        /**	Static constructor return a 4D non-uniform scale matrix,
        *	x,y,z,w: x, y, z, w scale. */
        ...
        *	axis: Axis of rotation. This is assumed to be normalized.
        *	angle: Angle of rotation in radians.
        *	NOTE: Positive values of angle rotate counter-clockwise about axis as per
        *	the right-hand rule.*/
        ...
        *	NOTE: q is a unit quaternion representing a rotation. */
        ...
        /**	Static constructor return a 3D rotation matrix (as an affine transformation).
        *	x, y, z: the rotation angles about the three axis (equal to glRotatef()) but
        *   in radians. */
        static Mat4<T> rotation(T x, T y, T z);
    ...
    *******************************************************************************/
    ...
    #endif
        return Mat4<T>(m1Data) * Mat4<T>(m2Data);
    }
