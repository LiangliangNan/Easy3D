...
alpha is made of four doubles q_[i]:
{q_[0],q_[1],q_[2]} = sin(alpha/2) * {axis[0],axis[1],axis[2]}
q_[3] = cos(alpha/2)
Note that certain implementations place the cosine term in first position (instead of last here).

The Quat is always normalized, so that its inverse() is actually its conjugate.
*/
...
namespace easy3d {
    ...
    public:
        ...
        FT x() const { return q_[0]; }
        FT y() const { return q_[1]; }
        FT z() const { return q_[2]; }
        FT w() const { return q_[3]; }
        FT& x() { return q_[0]; }
        FT& y() { return q_[1]; }
        FT& z() { return q_[2]; }
        FT& w() { return q_[3]; }
        ...
        FT operator[](int i) const { return q_[i]; }

        /* Bracket operator returning an l-value. i must range in [0..3]. */
        FT& operator[](int i) { return q_[i]; }
        ...
        {
            return Quat(
                a.q_[3]*b.q_[0] + b.q_[3]*a.q_[0] + a.q_[1]*b.q_[2] - a.q_[2]*b.q_[1],
                a.q_[3]*b.q_[1] + b.q_[3]*a.q_[1] + a.q_[2]*b.q_[0] - a.q_[0]*b.q_[2],
                a.q_[3]*b.q_[2] + b.q_[3]*a.q_[2] + a.q_[0]*b.q_[1] - a.q_[1]*b.q_[0],
                a.q_[3]*b.q_[3] - b.q_[0]*a.q_[0] - a.q_[1]*b.q_[1] - a.q_[2]*b.q_[2]
                );
        }
        ...
        Quat inverse() const { return Quat(-q_[0], -q_[1], -q_[2], q_[3]); }

        /* Inverses the Quaternion (same rotation angle(), but negated axis()).
        See also inverse(). */
        void invert() { q_[0] = -q_[0]; q_[1] = -q_[1]; q_[2] = -q_[2]; }
        ...
        normalized(). */
        FT normalize() {
            const FT norm = std::sqrt(q_[0] * q_[0] + q_[1] * q_[1] + q_[2] * q_[2] + q_[3] * q_[3]);
            for (int i=0; i<4; ++i)
                q_[i] /= norm;
            return norm;
        }
    ...
    template <class FT>	std::ostream& operator<<(std::ostream& os, const Quat<FT>& q);
    template <class FT>	std::istream& operator>>(std::istream& is, Quat<FT>& q);
    ...
    void Quat<FT>::set_axis_angle(const Vec3& axis, FT angle)
    {
        ...
        {
            // Null rotation
            q_[0] = FT(0);      q_[1] = FT(0);       q_[2] = FT(0);      q_[3] = FT(1);
        }
        else
        {
            const FT sin_half_angle = std::sin(angle / 2.0f);
            q_[0] = sin_half_angle*axis[0] / norm;
            q_[1] = sin_half_angle*axis[1] / norm;
            q_[2] = sin_half_angle*axis[2] / norm;
            q_[3] = (FT)std::cos(angle / 2.0f);
        }
    }
    ...
    {
        const FT q00 = FT(2.0l * q_[0] * q_[0]);
        const FT q11 = FT(2.0l * q_[1] * q_[1]);
        const FT q22 = FT(2.0l * q_[2] * q_[2]);

        const FT q01 = FT(2.0l * q_[0] * q_[1]);
        const FT q02 = FT(2.0l * q_[0] * q_[2]);
        const FT q03 = FT(2.0l * q_[0] * q_[3]);

        const FT q12 = FT(2.0l * q_[1] * q_[2]);
        const FT q13 = FT(2.0l * q_[1] * q_[3]);

        const FT q23 = FT(2.0l * q_[2] * q_[3]);
    ...
    void Quat<FT>::get_axis_angle(Vec3& axis, FT& angle) const
    {
        ...
        // We call std::acos(q_[3]), but q_[3] equaling to 1 can actually be e.g., 1.00000012.
        if (q_[3] > FT(1)) {
            const_cast<Quat*>(this)->normalize();
        }
        angle = FT(2.0) * std::acos(q_[3]);
        axis = Vec3(q_[0], q_[1], q_[2]);
    ...
    typename Quat<FT>::Vec3 Quat<FT>::axis() const
    {
        ...
        return (std::acos(q_[3]) <= M_PI / 2.0) ? res : -res;
    }
    ...
    FT Quat<FT>::angle() const
    {
        ...
        const FT angle = FT(2.0) * std::acos(q_[3]);
    ...
    Quat<FT> Quat<FT>::log()
    {
        FT len = std::sqrt(q_[0] * q_[0] + q_[1] * q_[1] + q_[2] * q_[2]);

        if (len < 1E-6)
            return Quat(q_[0], q_[1], q_[2], 0.0);
        else
        {
            // The normalize() is here to prevent failure introduced by numerical error.
            // We call std::acos(q_[3]), but q_[3] equaling to 1 can actually be e.g., 1.00000012.
            if (q_[3] > FT(1)) {
                const_cast<Quat*>(this)->normalize();
            }		FT coef = std::acos(q_[3]) / len;
            return Quat(q_[0] * coef, q_[1] * coef, q_[2] * coef, 0.0);
        }
    }
    ...
    Quat<FT> Quat<FT>::exp()
    {
        FT theta = std::sqrt(q_[0] * q_[0] + q_[1] * q_[1] + q_[2] * q_[2]);

        if (theta < 1E-6)
            return Quat(q_[0], q_[1], q_[2], std::cos(theta));
        else
        {
            FT coef = std::sin(theta) / theta;
            return Quat(q_[0] * coef, q_[1] * coef, q_[2] * coef, std::cos(theta));
        }
    }
