...
namespace interpolation {
     ...
     *              if (i > 0)
     *                  t += std::max(distance(kfi.keyframe(i-1).position(), p), float(1e-6));
     *              T[i] = t;
     *              X[i] = p.x;
     *              Y[i] = p.y;
     *              Z[i] = p.z;
     *          }
     *
     *          interpolation::spline x_spline;     x_spline.set_points(T, X);
     *          interpolation::spline y_spline;     y_spline.set_points(T, Y);
     *          interpolation::spline z_spline;     z_spline.set_points(T, Z);
