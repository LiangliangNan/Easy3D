...
namespace easy3d {
    ...
    /**
     * @brief A Graph data strucutre with easy property management.
     *
     * @todo 1) replace 'edges(Vertex v)' by 'VertexAroundVertexCirculator vertices(Vertex v) const' (see line 812)
     *       2) replace 'vertices(Vertex v)' by 'EdgeAroundVertexCirculator edges(Vertex v) const' (line 806)
     */
    ...
    //    /// this class circulates through all edges connceted with a vertex.
    //    /// it also acts as a container-concept for C++11 range-based for loops.
    //    /// \sa VertexAroundVertexCirculator
    //    class EdgeAroundVertexCirculator
    //    {
    //    public:

    //        /// default constructor
    //        EdgeAroundVertexCirculator(const Graph* m=nullptr, Vertex v=Vertex())
    //            : graph_(m)
    //            , current_(v)
    //            {
    //                if (graph_) iterator_ = graph_->edges(v).begin();
    //            }
    ...
    //        {
    //            assert(graph_);
    //            return (graph_==rhs.graph_) && (iterator_==rhs.iterator_);
    //        }
    ...
    //        Edge operator*() const { return *iterator_; }

    //        /// cast to bool: true if vertex is not isolated
    //        operator bool() const { return (!graph_->edges(current_).empty()); }

    //    private:
    //        const Graph*  graph_;
    //        const Vertex  current_;
