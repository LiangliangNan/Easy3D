...
SurfaceMesh* convert_to_mesh(CoredFileMeshData<Vertex>& mesh, const XForm4x4< REAL >& iXForm, const std::string& density_attr_name, bool has_colors) {
...
SurfaceMesh* PoissonReconstruction::apply(const PointCloud* cloud, const std::string& density_attr_name) {
	...
	{ // Load the samples (and color data)
		...
		xForm = XForm4x4< REAL >::Identity();
		{
			...
			REAL scale = std::max< REAL >(max_p[0] - min_p[0], std::max< REAL >(max_p[1] - min_p[1], max_p[2] - min_p[2]));
			scale *= scale_;
			for (int i = 0; i < 3; i++)
				center[i] -= scale / 2;
        ...
        pointCount = tree.init< Point3D< REAL > >(cloud->n_vertices(), pts, nms, cls, xForm, maxSolveDepth, false, *samples, sampleData);
...
#pragma omp parallel for num_threads(threads_)
	...
	DenseNodeData< REAL, DEGREE > solution;
	{
		...
		// Trim the tree and prepare for multi-grid
		{
			...
			tree.inalizeForBroodedMultigrid< MAX_DEGREE, DEGREE, BType >(full_depth_, typename Octree< REAL >::template HasNormalDataFunctor< NORMAL_DEGREE >(*normalInfo), &indexMap);
		...
		// Solve the linear system
		{
			...
			solverInfo.cgDepth = cgDepth_, solverInfo.iters = gsIter_, solverInfo.cgAccuracy = solverAccuracy, solverInfo.verbose = verbose_, solverInfo.showResidual = showResidual, solverInfo.lowResIterMultiplier = std::max< double >(1., 1.);
			solution = tree.solveSystem< DEGREE, BType >(FEMSystemFunctor< DEGREE, BType >(0, 1., 0), iInfo, constraints, solveDepth, solverInfo);
			if (verbose_)
