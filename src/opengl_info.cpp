...
bool OpenglInfo::init() {
	...
	std::cout << "GL Version: " << OpenglInfo::gl_version() << std::endl;
	std::cout << "GLSL Version: " << OpenglInfo::glsl_version() << std::endl;

	if (OpenglInfo::gl_version_number() >= 2.0 && OpenglInfo::gl_vendor().find("NVIDIA") != std::string::npos) {
		int size = OpenglInfo::total_gpu_memory();
		if (size >= 0)
			std::cout << "total GPU memory (MB): " << size << std::endl;
		size = OpenglInfo::available_gpu_memory();
		if (size >= 0)
			std::cout << "available GPU memory (MB): " << size << std::endl;
	}
...
int OpenglInfo::gl_profile() {
	...
	if (profile == GL_CONTEXT_CORE_PROFILE_BIT) {
		std::cout << "GL_CONTEXT_CORE_PROFILE_BIT" << std::endl;
	}
	else if (profile == GL_CONTEXT_COMPATIBILITY_PROFILE_BIT) {
		std::cout << "GL_CONTEXT_COMPATIBILITY_PROFILE_BIT" << std::endl;
	}
	else {
...
int OpenglInfo::total_gpu_memory() {
	if (OpenglInfo::is_supported("GL_NVX_gpu_memory_info")) {		// NVidia
		int size;
...
int OpenglInfo::available_gpu_memory() {
	if (OpenglInfo::is_supported("GL_NVX_gpu_memory_info")) {
		int size;
...
std::map<int, std::string>	OpenglInfo::spBufferAccess;
std::map<int, std::string>	OpenglInfo::spBufferUsage;
std::map<int, std::string>	OpenglInfo::spBufferBinding;
std::map<int, int>			OpenglInfo::spBoundBuffer;
std::map<int, int>			OpenglInfo::spBufferBound;
std::map<int, std::string>	OpenglInfo::spDataF;
std::map<int, std::string>	OpenglInfo::spGLSLType;
std::map<int, int> 			OpenglInfo::spGLSLTypeSize;
std::map<int, std::string>	OpenglInfo::spShaderType;
std::map<int, std::string>	OpenglInfo::spTransFeedBufferMode;
std::map<int, std::string>	OpenglInfo::spGLSLPrimitives;
std::map<int, std::string>	OpenglInfo::spTessGenSpacing;
std::map<int, std::string>	OpenglInfo::spVertexOrder;
