...
bool ShaderProgram::_read_shader_file(const std::string& file_name, std::string& shader_code) {
	shader_code.clear();
	...
	shader_code.resize(length);
	in.read(&(shader_code[0]), length);
#endif
...
bool ShaderProgram::load_shader_from_file(ShaderType type, const std::string& file_name) {
	std::string str;
	if (!_read_shader_file(file_name, str)) {
		std::cerr << "failed reading shader file \'" << file_name << "\'" << std::endl;
		return false;
	}
	return load_shader_from_code(type, str);
}
