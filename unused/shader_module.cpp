 ...
 */
    inline std::vector<std::string> precompile_shader(const std::string &source) {
        ...
        for (auto &line: lines) {
            if (line.find("#include \"") == 0) {
                ...
                //Todo: Liangliang: the path may not be correct (it should be w.r.t. the current path)
                auto include_file = precompile_shader(read_text_file(include_path));
    ...
    ShaderModule::ShaderModule(LveDevice &device, VkShaderStageFlagBits stage, const ShaderSource &glsl_source,
                               const std::string &entry_point, const ShaderVariant &shader_variant) :
            device{device},
            stage{stage},
            entry_point{entry_point} {
        debug_name = glsl_source.get_filename() +
                     "[variant: " + std::to_string(shader_variant.get_id()) + "] [entrypoint: " + entry_point + "]";
        ...
        if (it != resources.end()) {
            if (resource_mode == ShaderResourceMode::Dynamic) {
                if (it->type == ShaderResourceType::BufferUniform || it->type == ShaderResourceType::BufferStorage) {
                    ...
                    LOGE("Resource '{}' does not support dynamic", resource_name);
                }
            } else {
                it->mode = resource_mode;
            }
        } else {
            LOGE("Resource '{}' not found for shader", resource_name);
        }
    }
