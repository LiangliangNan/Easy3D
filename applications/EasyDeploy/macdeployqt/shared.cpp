...
void changeInstallName(const QString &bundlePath, const FrameworkInfo &framework, const QStringList &binaryPaths, bool useLoaderPath)
{
    ...
    foreach (const QString &binary, binaryPaths) {
        ...
        // Liangliang: fix the bug where the actually copied name is different from the canonicalFilePath()
        canonicalInstallName = QFileInfo(canonicalInstallName).absolutePath() + "/" + framework.binaryName;
