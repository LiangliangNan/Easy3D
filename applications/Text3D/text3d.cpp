...
//    }
    ...
    SurfaceMesh *Text3D::generate_mesh(const std::string& text, float extrude, unsigned short bezierSteps) const {
        if (!ready_)
            return nullptr;

        std::vector<Polygon2> contours;
        generate_contours(text, contours, bezierSteps);
    ...
    void Text3D::generate_contours(const std::string& text, std::vector<Polygon2>& contours, unsigned short bezierSteps) const {
        if (!ready_)
            return;

        float offset = 0;
        for (int i = 0; i < text.size(); i++) {
            offset = generate_contours(text[i], offset, contours, bezierSteps);
        }
    }


    float Text3D::generate_contours(char ch, float offset, std::vector<Polygon2>& contours, unsigned short bezierSteps) const {
        static FT_UInt prevCharIndex = 0, curCharIndex = 0;
        static FT_Pos prev_rsb_delta = 0;

        curCharIndex = FT_Get_Char_Index(get(face_), ch);
        if (FT_Load_Glyph(get(face_), curCharIndex, FT_LOAD_DEFAULT))
            printf("FT_Load_Glyph failed\n");
        ...
        short nCountour = 0;
        nCountour = get(face_)->glyph->outline.n_contours;

        int startIndex = 0, endIndex = 0;
        FT_Outline *o = &get(face_)->glyph->outline;

        if (FT_HAS_KERNING(get(face_)) && prevCharIndex) {
            FT_Vector kerning;
            FT_Get_Kerning(get(face_), prevCharIndex, curCharIndex, FT_KERNING_DEFAULT, &kerning);
            offset += kerning.x >> 6;
        }

        if (prev_rsb_delta - get(face_)->glyph->lsb_delta >= 32)
            offset -= 1.0f;
        else if (prev_rsb_delta - get(face_)->glyph->lsb_delta < -32)
            offset += 1.0f;

        prev_rsb_delta = get(face_)->glyph->rsb_delta;

        Vectoriser *vectoriser = new Vectoriser(get(face_)->glyph, bezierSteps);
        ...
        delete vectoriser;
        prevCharIndex = curCharIndex;
        offset += get(face_)->glyph->advance.x >> 6;
    }
