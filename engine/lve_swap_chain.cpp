...
namespace lve {
    ...
    LveSwapChain::LveSwapChain(LveDevice &deviceRef, const VkExtent2D& extent, bool vsync)
            : device{deviceRef}, windowExtent{extent} {
        init(vsync);
    }

    LveSwapChain::LveSwapChain(
            LveDevice &deviceRef, const VkExtent2D& extent, std::shared_ptr<LveSwapChain> previous, bool vsync)
            : device{deviceRef}, windowExtent{extent}, oldSwapChain{previous} {
        init(vsync);
        oldSwapChain = nullptr;
    }

    void LveSwapChain::init(bool vsync) {
    ...
    VkResult LveSwapChain::acquireNextImage(uint32_t *imageIndex) {
        vkWaitForFences(device.device(), 1, &inFlightFences[currentFrame], VK_TRUE, DEFAULT_FENCE_TIMEOUT);
    ...
    VkResult LveSwapChain::submitCommandBuffers(const VkCommandBuffer *buffers, uint32_t *imageIndex) {
        if (imagesInFlight[*imageIndex] != VK_NULL_HANDLE) {
            vkWaitForFences(device.device(), 1, &imagesInFlight[*imageIndex], VK_TRUE, DEFAULT_FENCE_TIMEOUT);
        }
    ...
    void LveSwapChain::createSwapChain(bool vsync) {
    ...
    VkPresentModeKHR LveSwapChain::chooseSwapPresentMode(bool vsync, const std::vector<VkPresentModeKHR> &availablePresentModes) {
